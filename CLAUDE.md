...
- **`omnimand`** (daemon) — long-running systemd user service. Owns the Tantivy index, the clipboard watcher, and exposes a D-Bus interface on `org.adrien.OmnimanDaemon` (zbus) with methods `Search`, `ClipboardHistory`, `AskAI`, `RequestShowUi` and a `ShowUi` signal.
...
| `omniman-clipboard` | wl-clipboard watcher + SQLite store (encrypted via Secret Service) |
...
- **Clipboard** uses `zwlr_data_control_manager_v1` (supported by Mutter ≥ 45). Verify at runtime; fall back to spawning `wl-paste --watch` if absent.
